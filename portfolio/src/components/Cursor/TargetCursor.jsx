...
const TargetCursor = ({
  ...
  // Handle special click for COPY since we are removing CustomCursor.jsx
  useEffect(() => {
    if (isMobile) return;
    const handleClick = (e) => {
      ...
      if (target) {
        ...
        // Optional visual feedback for copying
        if (dotRef.current) {
           gsap.to(dotRef.current, { scale: 3, backgroundColor: "#0F766E", duration: 0.2, yoyo: true, repeat: 1 });
        }
      }
    };
